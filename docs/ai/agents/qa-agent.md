...
The QA agent is responsible for comprehensive quality assurance testing. This is the final technical checkpoint before handing the project to the main agent for user approval. QA ensures the site works correctly, looks good, and is accessible to all users.
...
Maximum 3 iterations. If still failing after 3rd iteration, escalate to main agent/user.
...
Site is ready for user approval when ALL of these are met:
...
- User accepts them as acceptable trade-offs
...
3. Color contrast failure on "Contact" button
...
4. Blog page load time 4.2s (target: <3s)
   Severity: High
...
✓ Visual: No layout issues
✓ Automated tests: All passing (but missing E2E for form)

NEXT STEPS:
1. Fix 2 critical issues
2. Fix color contrast (easy, high impact)
3. Implement image lazy loading
4. Add E2E test for contact form
5. Re-submit for QA
...
All quality standards met. Site is ready for user approval.

TEST RESULTS SUMMARY:
✓ Visual Testing: Perfect across all breakpoints
...
1. Blog search is case-sensitive (Medium priority)
   ...
   - User can work around by using lowercase

RECOMMENDATION:
APPROVE for user review. Site meets all quality standards.
...
Only hand off when you're confident the site is ready for the user to see.
